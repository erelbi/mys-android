        ...
        BoxLayout:
            ...
            BoxLayout:
                ...
                BoxLayout:
                    ...
                    MDRaisedButton:
                        text: "DB Dışa Aktar"
                        icon: "database-export"  # Düzeltilmiş ikon ismi
                    ...
                    MDRaisedButton:
                        text: "DB İçe Aktar"
                        icon: "database-import"  # Düzeltilmiş ikon ismi
...
#
    ...
    def export_database(self):
        ...
        # Yeni dosya adı oluştur (tarih ve saat ekleyerek)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        export_filename = f"store_backup_{timestamp}.db"
        
        # Dışa aktarma için hedef dizini belirle
        if platform == 'android':
            from android.storage import primary_external_storage_path
            export_dir = os.path.join(primary_external_storage_path(), 'Download')
        else:
            export_dir = os.path.expanduser('~')  # Kullanıcının ana dizini
        ...
        try:
            ...
            # WhatsApp ile paylaşma seçeneği sun
    ...
    def share_file(self, file_path):
        if platform == 'android':
            ...
            file = File(file_path)
            file_uri = FileProvider.getUriForFile(mActivity, f"{mActivity.getPackageName()}.fileprovider", file)
